...
def generate_shift_masks(mask_path, batch_size):
    ...
    Phi_s_batch = torch.sum(Phi_batch**2, 1)  # 在通道维度上求平方和
...
def shift(inputs, step=2):
    """
    @params:
    inputs: 形状为 [bs, nC, row, col] 的张量，分别代表批次大小、颜色通道数、图像的高度和宽度。
    step: 用于定义每个通道在宽度上偏移的步长，默认为 2。
    output: 形状为 [bs, nC, row, col + (nC - 1) * step] 的张量，它包含了所有颜色通道按指定步长平移后的结果。
    """
